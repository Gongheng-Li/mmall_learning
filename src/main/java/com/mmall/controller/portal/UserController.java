...
//            session.setAttribute(Const.CURRENT_USER, response.getData());
            ...
            RedisPoolUtil.setEx(session.getId(), JsonUtil.obj2String(response.getData()), Const.RedisCacheExpireTime.REDIS_SESSION_EXPIRE_TIME);
        }
        return response;
    }
...
//        session.removeAttribute(Const.CURRENT_USER);
    ...
    public ServerResponse<User> updateInformation(HttpServletRequest request, User user) {
        ...
        if (response.isSuccess()) {
            RedisPoolUtil.setEx(loginToken, JsonUtil.obj2String(response.getData()), Const.RedisCacheExpireTime.REDIS_SESSION_EXPIRE_TIME);
        }
        return response;
    }
    ...
    @ResponseBody
    ServerResponse<User> getInformation(HttpServletRequest request) {
        String loginToken = CookieUtil.readLoginToken(request);
        if (StringUtils.isEmpty(loginToken)) {
            return ServerResponse.createByErrorMessage("用户未登录，无法获取当前用户信息");
        }
