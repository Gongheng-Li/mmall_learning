...
public class UserServiceImpl implements IUserService {
    ...
    public ServerResponse<String> forgetResetPassword(String username, String forgetToken, String passwordNew) {
        ...
        if (StringUtils.equals(forgetToken, token)) {
            ...
            int affectedRowCount = userMapper.updatePasswordByUsername(username, md5Password);
    ...
    public ServerResponse<String> resetPassword(String passwordOld, String passwordNew, User user) {
        //防止横向越权，要校验以下这个用户的旧密码，且必须指定这个用户，因为如果不指定id，那么查询count(1)的结果会是大于0
    ...
    public ServerResponse<User> updateInformation(User user) {
        //username是不能更新的
        //email也要进行校验,检查是否存在除本用户之外的用户使用了相同的email
        int resultCount = userMapper.checkEmailByUserId(user.getEmail(), user.getId());
        if (resultCount > 0) {
            return ServerResponse.createByErrorMessage("该email已存在，请尝试其他email");
        }
